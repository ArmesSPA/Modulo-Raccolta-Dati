...
document.getElementById("normativa").value = "UNI EN 16681:2016";
...
/*const input = document.getElementById('numeroPreventivo');
...
input.addEventListener('input', () => {
  ...
  for (let i = 0; i < val.length && i < 8; i++) {
    ...
    if (i < 2) {
      // primi 2 caratteri numeri
      if (/\d/.test(c)) newVal += c;
    } else if (i === 2) {
      // 3° carattere deve essere Q
      if (c === 'Q') newVal += c;
    } else if (i === 3) {
      // 4° carattere deve essere _
      if (c === '_') newVal += c;
    } else {
      // ultimi 5 caratteri numeri
      if (/\d/.test(c)) newVal += c;
    }
  }
...
form.addEventListener('submit', (e) => {
  const pattern = /^\d{2}Q_\d{5}$/;
  if (!pattern.test(input.value)) {
    e.preventDefault();
    alert('Formato non valido! Usa: 2 numeri, Q, underscore, 5 numeri. Es: 25Q_01345');
    input.focus();
  }
});*/
